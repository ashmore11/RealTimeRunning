...
class UIGraphView: UIView {
    ...
    func drawGrid(context:CGContextRef) {
        ...
        for var y = 1 ; y<Int(yMax) ; y++ {
            ...
            CGPathMoveToPoint(thePath, nil, 0.0, floor(self.bounds.size.height - scaledPoint)+0.5)
            CGPathAddLineToPoint(thePath,nil,CGFloat(self.bounds.size.width-1),floor(self.bounds.size.height - scaledPoint)+0.5)
        }
    ...
    func drawLine(context:CGContextRef, pointsArray:[Double], lineColor:UIColor)
    {
        ...
        CGPathMoveToPoint(thePath, nil, floor(x)+0.5, floor(self.bounds.size.height - scaledPoint)+0.5)
        ...
        for point in pointsArray {
            ...
            CGPathAddLineToPoint(thePath,nil,floor(x)+0.5,floor(self.bounds.size.height - scaledPoint)+0.5)
            x += xDelta
        }
        CGContextBeginPath(context);
        CGContextAddPath(context, thePath );
